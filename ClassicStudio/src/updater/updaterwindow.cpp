...
void UpdaterWindow::setUpdateUrl ()
{
    QString strModuleName = qApp->applicationName();
#ifdef Q_OS_WIN
#ifndef Q_OS_WIN64
    strModuleName += "-x86";
#else
...
#ifdef QT_DEBUG
    UPDATES_URL = QString("https://test79.ubtrobot.com/v1/upgrade-rest/version/upgradable?productName=AlphaMiniPC&moduleNames=%1&versionNames=%2")
            .arg(strModuleName)
            .arg(qApp->applicationVersion());
#else
    UPDATES_URL = QString("https://apis.ubtrobot.com/v1/upgrade-rest/version/upgradable?productName=AlphaMiniPC&moduleNames=%1&versionNames=%2")
            .arg(strModuleName)
            .arg(qApp->applicationVersion());
#endif
}
...
void UpdaterWindow::checkForUpdates(bool force)
{
    ...
    if(!m_updater->getUpdateAvailable(UPDATES_URL)){
        ...
        m_updater->setUseCustomInstallProcedures(UPDATES_URL, false);
...
void UpdaterWindow::resetControls()
{
    ...
    /* Set title label */
    if(m_updater->getUpdateAvailable(UPDATES_URL)){
        m_ui->title->setText(tr("有新版本可用!"));
    }else{
        m_ui->title->setText(tr("你是最新的!"));
    }
    ...
    if(m_ui->changelog->toPlainText().isEmpty()){
        m_ui->changelog->setText("<p>找不到更新日志...</p>");
    }
...
void UpdaterWindow::openDownload(const QString& file)
{
    ...
    m_ui->timeLabel->setText(tr("正在打开下载文件")+ "...");
